...
export const Navbar = () => {
  ...
  const{user,setUser,setShowUserLogin,navigate,setSearchQuery,searchQuery,getCardCount,axios,setCartItems}=useAppContext();
  ...
  <div className="absolute top-[60px] left-0 w-full bg-white shadow-md py-4 flex flex-col items-start gap-2 px-5 text-sm md:hidden z-50">
    ...
    {user && (
      <NavLink to='/orders' onClick={() => { setOpen(false); window.scrollTo(0, 0); }}>My Orders</NavLink>
    )}
    <NavLink to='/seller' onClick={() => { setOpen(false); window.scrollTo(0, 0); }}>Seller</NavLink>
